<x-app-layout>
    <main class=" pt-[95px] grid place-items-center gap-5 min-h-screen mb-2">
        <div class="bg-white rounded-xl shadow-xl max-w-4xl relative">
            ...
            <div class="py-12 px-5 flex flex-col gap-5">
                ...
                @else
                    <form method="POST" action="{{ $permitedAction ? route('forms.response') : '' }}"
                        class="space-y-4">
                        @csrf
                        @if ($permitedAction)
                        ...
                        @foreach ($form->fields as $field)
                            ...
                            @else
                                <x-input-label :for="$field->label" :value="$field->label" />
                                <x-select-input :id="$field->label" class="block mt-1 w-full" :options="array_combine(explode(',', $field->options), explode(',', $field->options))"
